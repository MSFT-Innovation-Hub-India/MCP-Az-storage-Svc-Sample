...
class AzureBlobStorageService:
    ...
    def __init__(self, storage_account_name: Optional[str] = None):
        ...
        if not self._storage_account_name:
            logger.warning("No Azure Storage account name provided in parameters or .env file. "
            ...
            try:
                ...
                # Create the BlobServiceClient using the updated API
                ...
                logger.info(f"Azure Blob Storage service initialized successfully using DefaultAzureCredential for account: {self._storage_account_name}")
        ...
        try:
            if tool_name == "list_containers":
                ...
                if not container_name or not blob_name or content is None:
                    raise ValueError("container_name, blob_name, and content are required")
    ...
    async def _list_containers(self) -> Dict[str, Any]:
        ...
        try:
            containers = []
            container_list = self._blob_service_client.list_containers(include_metadata=True)
    ...
    async def _create_container(self, container_name: str) -> Dict[str, Any]:
        ...
        try:
            container_client = self._blob_service_client.create_container(container_name)
            return {
                "success": True,
                "name": container_name,
                "created": True
    ...
    async def _list_blobs(self, container_name: str, prefix: str = "") -> Dict[str, Any]:
    ...
    async def _download_blob(self, container_name: str, blob_name: str) -> Dict[str, Any]:
